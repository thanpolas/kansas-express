...
Manage.prototype.addRoutes = function(app) {
  ...
  app.use(function(req, res, next) {
    console.log('REQUEST MIDDLEWARE ALL!!!!:', req);
    next();
  });
  console.log('addRoutes() :: prefix:', prefix);
  app.get(prefix + '/token', this.readAll);
  app.get(prefix + '/token/:token', this.readOne);
  app.post(prefix + '/token', this.create);
  app.del(prefix + '/token/:token', this.del);
};
...
Manage.prototype._del = function(req, res) {
  ...
  this.kansas.get(token).then(function(result) {
    ...
    return this.kansas.del(req.kansas)
      .then(function() {
        res.statusCode = 204;
        res.end();
      });
...
Manage.prototype._create = function(req, res) {
  console.log('_create() :: Init...');
  var self = this;
  this.kansas.create(req.kansas)
    .then(function(result) {
      console.log('_create() :: done:', result);
      self.options.handleSuccess(res, result);
    })
    .catch(function(err) {
      console.log('_create() :: Error:', err);
      self.options.handleError(res, err);
    });
};
...
Manage.prototype.handleSuccess = function(res, result) {
  console.log('handleSuccess() :: Init:', result);
  res.setHeader('Content-Type', 'application/json');
 ...
 */
Manage.prototype.getParams = function(req, res, next) {
  var self = this;
  console.log('getParams() :: Init:', typeof next);
  return Promise.try(this.options.provide)
    .then(function(params) {
      console.log('getParams() OK:', params);
      var error;
      ...
      console.log('getParams() :: req.kansas:', params);
      ...
      console.log('getParams() ERROR:', err);
